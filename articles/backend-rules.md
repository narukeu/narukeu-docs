...
- 应至少包含 `Controller`、`Service`、`Repository/DAO` 三层。
- `Service` 层应遵循接口先行原则，先定义接口再编写实现。
- 对于 Go 等无原生类与接口概念的语言，可通过定义接口类型与结构体实现类似的分层。
- 代码文件与目录结构应以功能模块为单位进行划分，确保各模块职责清晰，易于维护和扩展。
...
- **DTO (Data Transfer Object)**：用于层间（如客户端与服务端）数据传输的对象，通常作为 API 的输入参数。DTO 可命名为 `CreateUserDTO`。为复用公共字段，可定义 `BaseDTO` 供其他 DTO 继承，其结构可参考 `BaseModel`。
- **VO (View Object)**：服务端返回给客户端的视图层对象。若其结构与 DTO 相同，可复用。命名方式可为 `FindListVO`。
...
- `findList`、`findAll`、`findTree`、`findTreeAll` 接口必须支持名为 `mode` 的 URL 查询参数，用于控制是否包含逻辑删除的数据。
- `mode` 参数的可选值如下：
...
对于仅需一组资源 ID 的批量操作，建议统一使用“批量 ID 入参对象”，以提升接口的一致性与可读性。建议定义为公共 `DTO`，命名为 `ByIdsDTO`。
...
- `4xxx`：业务逻辑错误（包括因权限不足导致的 `withDeleted` 和 `onlyDeleted` 鉴权失败）
- `5xxx`：数据库相关错误
- `9xxx`：未知错误
...
- **公共字段**：所有数据表都应包含一组标准公共字段，以记录元数据。
  ...
  - `status` (状态)：`smallint` 类型，`0` 代表无效，`1` 代表有效。
  - `remark` (备注)：`text` 类型。
- **主键**：主键 ID 应使用 `uuidv7`。若数据库支持，可使用 `uuid_generate_v7()` 函数生成；否则，在应用层生成。
- **模型定义**：`BaseModel` 应包含上述公共字段，其他数据模型继承之。模型中必须显式定义这些字段，不依赖 ORM 的隐式功能。
- **状态说明**：`status` 字段表示资源自身的业务状态，与逻辑删除状态无关。逻辑删除通过 `deleted_time` 字段实现，两者职责分离。

#### 4.1.2. 关系 (Relation)

- **关系类型**：应根据业务需求选择合适的关系类型（一对一、一对多、多对多），避免滥用多对多关系。
- **不使用外键**：为避免高并发写入场景下的性能瓶颈，不使用数据库外键约束。数据一致性由应用层逻辑保障。
  - 由于不使用外键，应用层必须承担保障数据一致性的全部责任：
    - 关联写入操作必须置于事务中。
    - 删除或更新关联数据前，必须检查是否存在依赖关系，防止产生孤立数据。
    - 应定期运行数据完整性检查脚本，以发现并修复潜在的不一致问题。
...
- **权限模型**：后端项目应采用基于角色的访问控制（RBAC）模型。每个 API 方法都应映射到一个唯一的权限点。
- **权限点命名**：权限点标识符应与 API 方法名严格对应，以实现精细化控制。例如，`create` 方法对应 `create` 权限点。
- **查询模式权限**：
  - 查看已删除数据的能力应被视为一项特殊权限。
