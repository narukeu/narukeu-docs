...
一个示例的 `.golangci.yml` 配置文件可以参考以下内容：
...
项目结构遵循社区广泛认可的 [Standard Go Project Layout](https://github.com/golang-standards/project-layout/blob/master/README.md)。这并非强制标准，但为大型项目提供了一个优秀的起点。请根据项目规模和需求选择性地使用这些目录。
...
- `/vendor`: 项目依赖。通过 `go mod vendor` 命令创建。如果你的团队使用统一的代理或依赖管理策略，此目录可能不是必需的。
...
- **枚举从 1 开始**：由于变量的默认零值是 0，枚举通常应从非零值开始（使用 `iota + 1`），以避免将未初始化的变量误解为第一个有效的枚举值。除非零值本身就是期望的默认行为。
