...
> [!NOTE]
> 唯一性与软删除并存的表（如用户、角色、菜单、部门、字典等），一律采用“部分唯一索引（WHERE deleted_time IS NULL）”保障逻辑唯一，避免被软删除记录阻断重建。区分大小写的唯一性需求应在索引层统一（如对 `LOWER(name)` 建立唯一索引）。
...
接口集合与行为遵循《后端项目开发规范（通用）》中定义的“标准 CRUD 接口”和“当前用户相关接口”约定：

- 基础接口：`findOne`、`findList`、`findAll`、`create`、`createMany`、`update`、`delete`、`deleteMany`、`remove`、`removeMany`、`restore`、`restoreMany`、`count`；
...
角色模块沿用通用规范的标准接口集合：`findOne`、`findList`、`findAll`、`create`、`createMany`、`update`、`delete`、`deleteMany`、`remove`、`removeMany`、`restore`、`restoreMany`、`count`，参数与返回体结构不再赘述。
...
- `findAll` - 查询所有在线用户列表（慎用，仅用于管理端无需分页的场景）。
- `count` - 查询当前在线会话总数。
...
查询接口（`findList` / `findAll`）支持以下筛选字段（均为 JSON 字段，采用 `camelCase`）：
...
字典模块接口与菜单、部门模块类似，沿用通用规范中的标准列表/树/统计接口：`findOne`、`findList`、`findAll`、`findTree`、`findTreeAll`、`create`、`createMany`、`update`、`delete`、`deleteMany`、`remove`、`removeMany`、`restore`、`restoreMany`、`count`，此处不展开入参/出参细节。
...
部门模块接口沿用标准树表模块规范：`findOne`、`findList`、`findAll`、`findTree`、`findTreeAll`、`create`、`createMany`、`update`、`delete`、`deleteMany`、`remove`、`removeMany`、`restore`、`restoreMany`、`count`，树结构入参/返回规则参考通用规范，这里只在“查询逻辑”中强调 Ripple 特有行为。
...
- API 权限模型：
  ...
  - 查询模式相关权限：
    - 当请求参数 `mode=withDeleted` 时，需具备 `withDeleted` 权限点；
    - 当 `mode=onlyDeleted` 时，需具备 `onlyDeleted` 权限点；
    - `findAll` 接口由于可能返回大量数据，必须绑定单独的权限点 `findAll`，防止被滥用。
  - 软删除相关权限：
