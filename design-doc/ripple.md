...
- 当创建、修改（变更权限集合）、删除角色时，必须同步更新 Casbin 的策略存储（Policy Storage）。具体表现为：先清除该角色在 Casbin 中旧的 p (policy) 策略，再根据新的 menuIds 查出对应的 `permission_code`，重新写入 Casbin 规则。
...
- 若修改了菜单的 `permission_code`，必须同步更新 Casbin 中所有引用该权限码的策略记录，确保鉴权规则与元数据保持一致。
...
| module | VARCHAR(64)  | 所属模块 | 如 `user`、`role`、`menu` 等               | 是   |      | 否       |        |
...
- 这些初始化权限点应与菜单表中的 `permission_code` 保持一致；
...
- **权限点来源统一**：
  ...
  - 菜单表中的 `permission_code` 和 Casbin 策略中的 `obj` 字段仅能引用 `system_api_perm.code`，不允许在代码或配置中直接拼接字符串常量作为权限点；
...
- **与角色/菜单/Casbin 的关系**：
  - 角色通过角色-菜单关联间接获得权限点，菜单的 `permission_code` 必须引用 `system_api_perm.code`；
  - 当角色的 `menuIds` 集合发生变更时，Service 层应根据新的菜单集合查出对应的 `permission_code`，在写入 Casbin 策略前校验这些编码均存在于 `system_api_perm`；
...
- API 权限模型：
  ...
  - 鉴权实现方式：
    ...
    - 中间件判定逻辑：
      ...
      - 获取当前请求对应的 API 权限点（例如 `system:user:list`）。
      - 遍历用户拥有的每一个角色，调用 `Enforce(role, permission_code, "allow")`（此时 `sub` 即为角色编码）。
...
为适配 Ripple 的动态菜单权限设计，Casbin 的配置文件约定如下：
...
[matchers]
# 只要角色匹配，且权限编码匹配，即通过。
# act 在此处作为占位符，通常固定为 "allow" 或具体的 HTTP Method。
# 鉴于 Ripple 规范要求每个 API 绑定唯一 permission_code，建议将 act 固定为 "allow" 或忽略 act。
